...
const fetchRecommendedPlaylists = async () =>{
  try{
    ...
    recommendedPlaylists.value = (res.result || []).map((item) =>({
      id:item.id,
      title:item.name,
...
const fetchRecommendedSongs = async () =>{
  try{
    ...
    recommendedSongs.value = (res.result || []).map((item) =>({
      id:item.id,
      name:item.name,
      cover:item.picUrl,
...
const handleSongClick = (songId,index) =>{
  if (!songId) return
  playerStore.playSong(songId,index,recommendedSongs.value)
}
...
<template>
  ...
  <div class="hall-wrapper">
    <div class="hall-inner">
      ...
      <PlaylistList :playlists="recommendedPlaylists"></PlaylistList>
      <!-- 推荐新音乐 -->
      ...
      <ul class="song-list">
        <li 
          ...
          :key="song.id"
          @click="handleSongClick(song.id,index)"
          >
        ...
        <div class="singer-carousel-track">
          <div
            ...
            <ul class="singer-list">
              <li v-for="singer in slide" :key="singer.id" class="singer-item" @click="handleSingerClick(singer.id)">
