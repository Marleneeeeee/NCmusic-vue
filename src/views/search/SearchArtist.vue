...
const fetchResults = async () => {
  ...
  try {
    ...
    if (res.code === 200 && res.result) {
      ...
      const newArtists = (data.artists||[]).map((s)=>({
        id: s.id,
        name: s.name,
        avatar: s.img1v1Url,
        musicNum: s.musicSize,
        albumNum: s.albumSize,
        mvNum: s.mvSize
      }))
      items.value = [...(items.value || []), ...newArtists]
      offset.value += limit
      if ((data.artists?.length || 0) < limit) hasMore.value = false
    } else {
      hasMore.value = false
    }
  } catch (err) {
    console.error('搜索歌手出错', err)
  } finally {
    loading.value = false
