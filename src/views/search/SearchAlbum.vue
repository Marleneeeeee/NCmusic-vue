...
const fetchResults = async () => {
  ...
  try {
    ...
    if (res.code === 200 && res.result) {
      ...
      const newAlbums = (data.albums || []).map((al)=> ({
        id: al.id,
        title: al.name,
        artist: (al.ar||al.artists||[]).map((a)=>a.name).join('/'),
        size: al.size,
        cover: al.picUrl,
        desc: al.description,
        publishTime: al.publishTime
      }))
      items.value = [...(items.value || []), ...newAlbums]
      offset.value += limit
      if ((data.albums?.length || 0) < limit) hasMore.value = false
    } else {
      hasMore.value = false
    }
  } catch (err) {
    console.error('搜索专辑出错', err)
  } finally {
    loading.value = false
